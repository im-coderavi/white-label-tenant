process.env.NODE_ENV = 'test';
process.env.JWT_ACCESS_SECRET = 'test-access-secret-please-ignore-1234567890';
process.env.MONGO_URI = 'mongodb://127.0.0.1:27017/placeholder';
process.env.REFRESH_TOKEN_TTL_DAYS = '30';
process.env.CLOUDINARY_URL = 'cloudinary://test_key:test_secret@test_cloud';
process.env.SMTP_HOST = 'smtp.test.local';
process.env.SMTP_PORT = '587';
process.env.SMTP_USER = 'test@toolzypro.local';
process.env.SMTP_PASSWORD = 'test-password';
process.env.SMTP_FROM = 'noreply@toolzypro.local';
process.env.MOCK_WEBHOOK_SECRET = 'test-webhook-secret-please-ignore';
process.env.SECRET_ENCRYPTION_KEY = 'test-secret-encryption-key-1234567890';
process.env.RAZORPAY_KEY_ID = '';
process.env.RAZORPAY_KEY_SECRET = '';
process.env.PLATFORM_TARGET_IP = '127.0.0.1';
process.env.PLATFORM_CNAME_TARGET = 'toolzypro.in';
